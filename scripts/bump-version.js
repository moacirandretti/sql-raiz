...
function determineVersionType(prTitle) {
  ...
  if (titleUpper.includes('PATCH') || titleUpper.includes('HOTFIX')) {
    return 'PATCH';
  }
...
function createGitTag(version) {
  try {
    // Configura git para permitir commit
    execSync('git config user.name "github-actions[bot]"', { stdio: 'inherit' });
    execSync('git config user.email "github-actions[bot]@users.noreply.github.com"', { stdio: 'inherit' });
    
    // Adiciona e commita as mudanças no package.json
...
module.exports = { determineVersionType, incrementVersion, parseVersion };
